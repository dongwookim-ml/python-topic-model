...
class diln:
    ...
    def update_mean_Kernel(self, corpus):
        ...
        self.Kern = (np.dot((corpus.mu-self.mean).T, (corpus.mu-self.mean)) + np.diag(np.sum(corpus.sigma, 0)))/corpus.M
    ...
    def update_C(self, corpus, is_heldout):
        ...
        if(self.is_compute_lb):
            ...
            l1 = self.K * gammaln(self.dir_prior*self.N) - self.K * self.N * gammaln(self.dir_prior) - np.sum((self.dir_prior-1)*(psiGamma-psiGammaSum))
            lb += l1
            #entropy of q(eta)
            l2 = np.sum(gammaln(gammaSum)) - np.sum(gammaln(self.gamma)) + np.sum((self.gamma - 1)*(psiGamma - psiGammaSum))
            lb -= l2
    ...
    def update_Z(self, corpus):
        ...
        if(self.is_compute_lb):
            ...
            l1 = np.sum(-bp*corpus.mu) + np.sum((bp-1)*(E_ln_Z)) - np.sum(np.exp((-corpus.mu + 0.5*corpus.sigma))*corpus.A/corpus.B) - corpus.M*np.sum(gammaln(bp))
            lb += l1
            #entropy of q(Z)
            l2 = np.sum(corpus.A*np.log(corpus.B)) + np.sum((corpus.A-1)*(E_ln_Z)) - np.sum(corpus.A) - np.sum(gammaln(corpus.A))
            lb -= l2
    ...
    def update_W(self, corpus):
        ...
        for m in xrange(corpus.M):
            gradMU = - bp + (adivb[m,:])*np.exp(-corpus.mu[m,:]+ 0.5*corpus.sigma[m,:]) - np.dot(self.invKern, (corpus.mu[m,:]-self.mean))
            gradV = -0.5*(adivb[m,:]) * np.exp(-corpus.mu[m,:] + 0.5*corpus.sigma[m,:]) - .5*np.diag(self.invKern) + .5/corpus.sigma[m,:]
            stepsize = self.getstepMUV(corpus.mu[m,:],corpus.sigma[m,:],gradMU,gradV, bp, adivb[m,:], self.mean, self.invKern)
        ...
        if self.is_compute_lb:
            ...
            l1 = -0.5*np.sum(np.diag(np.dot(np.dot(corpus.mu-self.mean, self.invKern), (corpus.mu-self.mean).T ) )) - 0.5 * np.sum(np.diag(self.Kern) * corpus.sigma)
            lb += l1
            #entropy of q(w)
    ...
    def update_V(self, corpus):
        ...
        if self.is_compute_lb:
            #expectation of p(V)
            lb += (self.K-1)*gammaln(self.alpha+1) - (self.K-1)*gammaln(self.alpha) + np.sum((self.alpha-1)*np.log(1-self.V[:-1]))
    ...
    def update_alpha(self):
        self.alpha = (eps + self.K -2)/(self.alpha_2 - np.sum(np.log(1-self.V[:-1]+eps)))
    ...
    def getstepMUV(self, currMu, currV, vecMu, vecV, bp, AdivB, u, invKern):
        ...
        for ite in xrange(len(step_check_vec)):
            ...
            f[ite] = - np.sum(mu_check * bp) - np.sum(AdivB * np.exp(-mu_check + .5*v_check)) - .5*np.dot((mu_check-u), np.dot(invKern,(mu_check-u))) - np.dot(.5*np.diag(invKern), v_check) + .5*np.sum(np.log(v_check+eps))
        ...
        if b == len(step_check_vec):
            ...
            while bool:
                ...
                v_check[v_check>200] = 200
                fnew = - np.sum(mu_check * bp) - np.sum(AdivB * np.exp(-mu_check + .5*v_check)) - .5*np.dot((mu_check-u), np.dot(invKern,(mu_check-u))) - np.dot(.5*np.diag(invKern), v_check) + .5*np.sum(np.log(v_check+eps))
                if fnew > fold:
                    fold = fnew
                else:
                    bool = 0
    ...
    def write_corr_topics(self, corpus, filepath, thr=-1e100):
        with open(filepath, 'w') as f:
            ...
            for ti in xrange(self.K-1):
                for ki in xrange(ti+1,self.K):
                    if Kern[ti,ki] > thr:
