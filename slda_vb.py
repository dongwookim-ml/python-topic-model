...
class sLDA:
    ...
    def __init__(self, docs, responses, vocab, num_topic, alpha = 0.1, sigma=1):
        ...
        self.beta = np.random.gamma(100., 1./100, (self.K, self.W))
        ...
        self.alpha = alpha
        self.dir_prior = 0.01
        self.ssword = np.random.gamma(100., 1./100, (self.K, self.W))
        self.EA = np.zeros((self.D,self.K))
        self.EAA = np.zeros((self.K,self.K))
        self.small_iter = 10
    ...
    def do_e_step(self):
        ...
        for di in xrange(self.D):
            ...
            rss = (2. * np.dot(self.eta, phi_sum)[:,np.newaxis] * self.eta + (self.eta ** 2))/(2.* (np.float(doc_len)**2.) * (self.sigma**2.) )
            rss = rss.T     # K x len
            phi = np.exp(e_log_theta[:,np.newaxis] + e_log_beta[:,doc] + ((y/(np.float(doc_len)*(self.sigma**2.)))*self.eta)[:,np.newaxis] - rss + eps)
            phi /= np.sum(phi, 0)
    ...
    def heldoutEstep(self, max_iter, heldout):
        gamma = np.random.gamma(100., 1./100, (len(heldout), self.K))
